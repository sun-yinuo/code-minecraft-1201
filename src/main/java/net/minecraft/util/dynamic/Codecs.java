...
public class Codecs {
	...
	public static final Codec<TagEntryId> TAG_ENTRY_ID = Codec.STRING
		.comapFlatMap(
			tagEntry -> tagEntry.startsWith("#")
					? Identifier.validate(tagEntry.substring(1)).map(id -> new TagEntryId(id, true))
					: Identifier.validate(tagEntry).map(id -> new TagEntryId(id, false)),
			TagEntryId::asString
		);
