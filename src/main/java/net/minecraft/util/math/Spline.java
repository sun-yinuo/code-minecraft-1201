...
public interface Spline<C, I extends ToFloatFunction<C>> extends ToFloatFunction<C> {
	...
	Spline<C, I> apply(Visitor<I> visitor);
	...
	static <C, I extends ToFloatFunction<C>> Codec<Spline<C, I>> createCodec(Codec<I> locationFunctionCodec) {
		...
		Codec<Implementation<C, I>> codec2 = RecordCodecBuilder.create(
			instance -> instance.group(
						locationFunctionCodec.fieldOf("coordinate").forGetter(Implementation::locationFunction),
					...
					.apply(instance, (locationFunction, splines) -> {
						...
						return Implementation.build((I)locationFunction, fs, builder.build(), gs);
					})
		);
		mutableObject.setValue(
			Codec.either(Codec.FLOAT, codec2)
				.xmap(
					either -> either.map(FixedFloatFunction::new, spline -> spline),
					spline -> spline instanceof Spline.FixedFloatFunction<C, I> fixedFloatFunction
							? Either.left(fixedFloatFunction.value())
							: Either.right((Implementation)spline)
				)
		);
	...
	static <C, I extends ToFloatFunction<C>> Builder<C, I> builder(I locationFunction) {
		return new Builder<>(locationFunction);
	}

	static <C, I extends ToFloatFunction<C>> Builder<C, I> builder(I locationFunction, ToFloatFunction<Float> amplifier) {
	...
	public static record Implementation<C, I extends ToFloatFunction<C>>(
		...
		static <C, I extends ToFloatFunction<C>> Implementation<C, I> build(
