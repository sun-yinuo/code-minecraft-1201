...
public class WeightedList<U> implements Iterable<U> {
	...
	public WeightedList() {
		this.entries = Lists.<Entry<U>>newArrayList();
	}

	private WeightedList(List<Entry<U>> list) {
		this.entries = Lists.<Entry<U>>newArrayList(list);
	}
	...
	public static class Entry<T> {
		...
		public static <E> Codec<Entry<E>> createCodec(Codec<E> codec) {
			return new Codec<Entry<E>>() {
				@Override
				public <T> DataResult<Pair<Entry<E>, T>> decode(DynamicOps<T> ops, T data) {
					Dynamic<T> dynamic = new Dynamic<>(ops, data);
					return dynamic.get("data")
						.flatMap(codec::parse)
						.map(datax -> new Entry<>(datax, dynamic.get("weight").asInt(1)))
