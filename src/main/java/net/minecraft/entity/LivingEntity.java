...
public abstract class LivingEntity extends Entity implements Attackable {
	...
	private boolean tryUseTotem(DamageSource source) {
		//如果伤害来自于"BYPASSES_INVULNERABILITY",例如\kill指令
		if (source.isIn(DamageTypeTags.BYPASSES_INVULNERABILITY)) {
			...
			for (Hand hand : Hand.values()) {
				...
				if (itemStack2.isOf(Items.TOTEM_OF_UNDYING)) {
					...
					//递减此项堆栈中的项计数(参见net.minecraft.item.ItemStack)
			...
			//手上拿东西了吗？
			if (itemStack != null) {
				//是ServerPlayerEntity吗，同时强转ServerPlayerEntity类型
				if (this instanceof ServerPlayerEntity serverPlayerEntity) {
					//"Stats.USED.getOrCreateStat(Items.TOTEM_OF_UNDYING)"统计项增加
