...
public class RabbitEntity extends AnimalEntity implements VariantHolder<RabbitEntity.RabbitType> {
	...
	public void mobTick() {
		...
		if (this.isOnGround()) {
			...
			if (this.getVariant() == RabbitType.EVIL && this.ticksUntilJump == 0) {
	...
	public EntityData initialize(
		...
		RabbitType rabbitType = getTypeFromPos(world, this.getBlockPos());
	...
	public static enum RabbitType implements StringIdentifiable {
		...
		private static final IntFunction<RabbitType> BY_ID = ValueLists.createIdToValueFunction(RabbitType::getId, values(), BROWN);
		public static final com.mojang.serialization.Codec<RabbitType> CODEC = StringIdentifiable.createCodec(RabbitType::values);
		final int id;
