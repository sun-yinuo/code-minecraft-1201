...
public class InteractionEntity extends Entity implements Attackable, Targeter {
	...
	protected void readCustomDataFromNbt(NbtCompound nbt) {
		...
		if (nbt.contains("attack")) {
			Interaction.CODEC
				...
				.ifPresent(pair -> this.attack = (Interaction)pair.getFirst());
		} else {
			this.attack = null;
		}
		...
		if (nbt.contains("interaction")) {
			Interaction.CODEC
				...
				.resultOrPartial(Util.addPrefix("Interaction entity", field_42624::error))
				.ifPresent(pair -> this.interaction = (Interaction)pair.getFirst());
		} else {
	...
	protected void writeCustomDataToNbt(NbtCompound nbt) {
		...
		if (this.attack != null) {
			Interaction.CODEC.encodeStart(NbtOps.INSTANCE, this.attack).result().ifPresent(nbtElement -> nbt.put("attack", nbtElement));
		}

		if (this.interaction != null) {
			Interaction.CODEC.encodeStart(NbtOps.INSTANCE, this.interaction).result().ifPresent(nbtElement -> nbt.put("interaction", nbtElement));
		}
