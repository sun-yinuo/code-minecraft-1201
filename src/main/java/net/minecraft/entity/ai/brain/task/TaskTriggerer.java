 ...
 * </ol>
 * 
 * <p>A lambda is passed to the {@code task} method. This takes a {@link TaskContext}.
 * You can either call {@link TaskContext#point} to set the actual task function,
 ...
 * {@link TaskContext#getValue}.
 * 
 * <p>There are three query types:
 * 
 * <ul>
 * <li>{@link TaskContext#queryMemoryAbsent} that succeeds if a value is
 * <strong>not</strong> present in the memory.</li>
 * <li>{@link TaskContext#queryMemoryValue} that succeeds if a value is
 * present in the memory. The result is the queried value.</li>
 * <li>{@link TaskContext#queryMemoryOptional} that always succeeds. The value
...
public class TaskTriggerer<E extends LivingEntity, M> implements App<TaskTriggerer.K1<E>, M> {
	...
	public static <E extends LivingEntity> SingleTickTask<E> task(
		Function<TaskContext<E>, ? extends App<K1<E>, TaskRunnable<E>>> creator
	) {
		final TaskFunction<E, TaskRunnable<E>> taskFunction = getFunction((App<K1<E>, TaskRunnable<E>>)creator.apply(newContext()));
	...
	public static final class TaskContext<E extends LivingEntity> implements Applicative<K1<E>, TaskContext.Mu<E>> {
		...
		public <A, R> Function<App<K1<E>, A>, App<K1<E>, R>> lift1(App<K1<E>, Function<A, R>> app) {
			return app2 -> {
				final TaskFunction<E, A> taskFunction = (TaskFunction<E, A>)TaskTriggerer.getFunction(app2);
				final TaskFunction<E, Function<A, R>> taskFunction2 = TaskTriggerer.getFunction(app);
		...
		public <T, R> TaskTriggerer<E, R> map(Function<? super T, ? extends R> function, App<K1<E>, T> app) {
		...
		public <A, B, R> TaskTriggerer<E, R> ap2(
                App<K1<E>, BiFunction<A, B, R>> app, App<K1<E>, A> app2, App<K1<E>, B> app3
		) {
		...
		public <T1, T2, T3, R> TaskTriggerer<E, R> ap3(
			App<K1<E>, Function3<T1, T2, T3, R>> app,
			App<K1<E>, T1> app2,
			App<K1<E>, T2> app3,
			App<K1<E>, T3> app4
		) {
		...
		public <T1, T2, T3, T4, R> TaskTriggerer<E, R> ap4(
			App<K1<E>, Function4<T1, T2, T3, T4, R>> app,
			App<K1<E>, T1> app2,
			App<K1<E>, T2> app3,
			App<K1<E>, T3> app4,
			App<K1<E>, T4> app5
