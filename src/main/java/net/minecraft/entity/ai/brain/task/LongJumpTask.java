...
public class LongJumpTask<E extends MobEntity> extends MultiTickTask<E> {
	...
	protected List<Target> targets = Lists.<Target>newArrayList();
	...
	protected void run(ServerWorld serverWorld, E mobEntity, long l) {
		...
		int j = blockPos.getY();
		int k = blockPos.getZ();
		this.targets = (List<Target>)BlockPos.stream(
			...
			.map(blockPos2 -> new Target(blockPos2.toImmutable(), MathHelper.ceil(blockPos.getSquaredDistance(blockPos2))))
