...
public class PrepareRamTask<E extends PathAwareEntity> extends MultiTickTask<E> {
	...
	protected void keepRunning(ServerWorld serverWorld, E pathAwareEntity, long l) {
		if (!this.ram.isEmpty()) {
			pathAwareEntity.getBrain().remember(MemoryModuleType.WALK_TARGET, new WalkTarget(((Ram)this.ram.get()).getStart(), this.speed, 0));
			pathAwareEntity.getBrain().remember(MemoryModuleType.LOOK_TARGET, new EntityLookTarget(((Ram)this.ram.get()).getEntity(), true));
			...
			if (bl) {
				...
				if (blockPos.equals(((Ram)this.ram.get()).getStart())) {
					...
					if (l - (Long)this.prepareStartTime.get() >= (long)this.prepareTime) {
						pathAwareEntity.getBrain().remember(MemoryModuleType.RAM_TARGET, this.calculateRamTarget(blockPos, ((Ram)this.ram.get()).getEnd()));
	...
	private void findRam(PathAwareEntity entity, LivingEntity target) {
		...
		this.ram = this.findRamStart(entity, target).map(start -> new Ram(start, target.getBlockPos(), target));
	}
