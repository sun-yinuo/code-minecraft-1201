 ...
 * </div>
 */
public final class ItemStack implements FabricItemStack {
	 ...
	 * stacks, and the {@linkplain UnbreakingEnchantment
	 ...
	 * stack, and increment {@link Stats#BROKEN} if the stack is held
	...
	public List<Text> getTooltip(@Nullable PlayerEntity player, TooltipContext context) {
		...
		if (isSectionVisible(i, TooltipSection.ADDITIONAL)) {
			this.getItem().appendTooltip(this, player == null ? null : player.getWorld(), list, context);
		}

		if (this.hasNbt()) {
			if (isSectionVisible(i, TooltipSection.UPGRADES) && player != null) {
				ArmorTrim.appendTooltip(this, player.getWorld().getRegistryManager(), list);
			}

			if (isSectionVisible(i, TooltipSection.ENCHANTMENTS)) {
			...
			if (this.nbt.contains("display", NbtElement.COMPOUND_TYPE)) {
				...
				if (isSectionVisible(i, TooltipSection.DYE) && nbtCompound.contains("color", NbtElement.NUMBER_TYPE)) {
		...
		if (isSectionVisible(i, TooltipSection.MODIFIERS)) {
		...
		if (this.hasNbt()) {
			if (isSectionVisible(i, TooltipSection.UNBREAKABLE) && this.nbt.getBoolean("Unbreakable")) {
				list.add(Text.translatable("item.unbreakable").formatted(Formatting.BLUE));
			}

			if (isSectionVisible(i, TooltipSection.CAN_DESTROY) && this.nbt.contains("CanDestroy", NbtElement.LIST_TYPE)) {
			...
			if (isSectionVisible(i, TooltipSection.CAN_PLACE) && this.nbt.contains("CanPlaceOn", NbtElement.LIST_TYPE)) {
