...
public class PalettedContainer<T> implements PaletteResizeListener<T>, ReadableContainer<T> {
	...
	public static <T> Codec<PalettedContainer<T>> createPalettedContainerCodec(
		IndexedIterable<T> idList, Codec<T> entryCodec, PaletteProvider paletteProvider, T defaultValue
	) {
		Reader<T, PalettedContainer<T>> reader = PalettedContainer::read;
	...
	public static <T> Codec<ReadableContainer<T>> createReadableContainerCodec(
		...
		Reader<T, ReadableContainer<T>> reader = (idListx, paletteProviderx, serialized) -> read(idListx, paletteProviderx, serialized)
				.map(result -> result);
		return createCodec(idList, entryCodec, paletteProvider, defaultValue, reader);
	}
	...
	private static <T, C extends ReadableContainer<T>> Codec<C> createCodec(
		IndexedIterable<T> idList, Codec<T> entryCodec, PaletteProvider provider, T defaultValue, Reader<T, C> reader
	) {
		return RecordCodecBuilder.create(
				instance -> instance.group(
							entryCodec.mapResult(Codecs.orElsePartial(defaultValue)).listOf().fieldOf("palette").forGetter(Serialized::paletteEntries),
							Codec.LONG_STREAM.optionalFieldOf("data").forGetter(Serialized::storage)
						)
	 ...
	 */
	private Data<T> getCompatibleData(@Nullable PalettedContainer.Data<T> previousData, int bits) {
	...
	private static <T> DataResult<PalettedContainer<T>> read(
		IndexedIterable<T> idList, PaletteProvider paletteProvider, Serialized<T> serialized
	) {
	...
	public Serialized<T> serialize(IndexedIterable<T> idList, PaletteProvider paletteProvider) {
		...
		try {
			...
			var12 = new Serialized(biMapPalette.getElements(), optional);
		} finally {
			this.unlock();
		}
	 ...
	 */
	static record Data<T>(DataProvider<T> configuration, PaletteStorage storage, Palette<T> palette) {
		...
		public static final PaletteProvider BLOCK_STATE = new PaletteProvider(4) {
			...
			public <A> DataProvider<A> createDataProvider(IndexedIterable<A> idList, int bits) {
				return switch (bits) {
					...
					default -> new DataProvider(PaletteProvider.ID_LIST, MathHelper.ceilLog2(idList.size()));
				};
			}
		};
		/**
