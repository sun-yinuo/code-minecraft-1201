...
public final class ChunkSkyLightProvider extends ChunkLightProvider<SkyLightStorage.Data, SkyLightStorage> {
	private static final long field_44743 = class_8531.packWithAllDirectionsSet(15);
	private static final long field_44744 = class_8531.packWithOneDirectionCleared(15, Direction.UP);
	private static final long field_44745 = class_8531.method_51574(15, false, Direction.UP);
		...
		if (this.lightStorage.hasSection(l)) {
			...
			if (bl) {
				...
				if (n > 0) {
					...
					this.method_51565(blockPos, class_8531.packWithAllDirectionsSet(n));
				} else {
	...
	protected void method_51531(long blockPos, long l, int lightLevel) {
		...
		for (Direction direction : DIRECTIONS) {
			if (class_8531.isDirectionBitSet(l, direction)) {
				...
				if (this.lightStorage.hasSection(ChunkSectionPos.fromBlockPos(m))) {
					...
					if (k > j) {
						...
						if (n > j) {
							if (blockState == null) {
								blockState = class_8531.isTrivial(l) ? Blocks.AIR.getDefaultState() : this.getStateForLighting(this.field_44746.set(blockPos));
							}

							if (!this.shapesCoverFullCube(blockPos, blockState, m, blockState2, direction)) {
								this.lightStorage.set(m, n);
								if (n > 1) {
									this.method_51566(m, class_8531.method_51574(n, isTrivialForLighting(blockState2), direction.getOpposite()));
								}
	...
	protected void method_51530(long blockPos, long l) {
		int i = this.getNumberOfSectionsBelowPos(blockPos);
		int j = class_8531.getLightLevel(l);
				...
				if (this.lightStorage.hasSection(ChunkSectionPos.fromBlockPos(m))) {
					...
					if (k != 0) {
						if (k <= j - 1) {
							...
							this.method_51565(m, class_8531.packWithOneDirectionCleared(k, direction.getOpposite()));
							this.method_51587(m, direction, k, false, i);
						} else {
							this.method_51566(m, class_8531.method_51579(k, false, direction.getOpposite()));
						}
					}
				}
			}
					...
					if (!this.lightStorage.hasSection(ChunkSectionPos.asLong(m, o, n))) {
						...
						for (int r = 15; r >= 0; r--) {
							...
							if (bl) {
								...
								if (lightLevel > 1) {
									this.method_51566(s, class_8531.method_51574(lightLevel, true, direction.getOpposite()));
								}
							} else {
								this.lightStorage.set(s, 0);
								this.method_51565(s, class_8531.packWithOneDirectionCleared(lightLevel, direction.getOpposite()));
							}
						}
						...
						if (t <= q) {
							...
							for (int z = q; z >= Math.max(p, t); z--) {
								...
								if (z == t || z < y) {
									...
									this.method_51566(aa, class_8531.method_51578(z == t, z < u, z < v, z < w, z < x));
								}
							}
