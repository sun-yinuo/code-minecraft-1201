...
public class MultiNoiseBiomeSourceParameterList {
	...
	public static Map<Preset, MultiNoiseUtil.Entries<RegistryKey<Biome>>> getPresetToEntriesMap() {
		return (Map<Preset, MultiNoiseUtil.Entries<RegistryKey<Biome>>>) Preset.BY_IDENTIFIER
			.values()
			.stream()
	...
	public static record Preset(Identifier id, BiomeSourceFunction biomeSourceFunction) {
		public static final Preset NETHER = new Preset(
			new Identifier("nether"),
			new BiomeSourceFunction() {
				@Override
		...
		public static final Preset OVERWORLD = new Preset(
			new Identifier("overworld"), new BiomeSourceFunction() {
				@Override
				public <T> MultiNoiseUtil.Entries<T> apply(Function<RegistryKey<Biome>, T> function) {
		...
		static final Map<Identifier, Preset> BY_IDENTIFIER = (Map<Identifier, Preset>)Stream.of(
				NETHER, OVERWORLD
			)
			.collect(Collectors.toMap(Preset::id, preset -> preset));
		public static final Codec<Preset> CODEC = Identifier.CODEC
			.flatXmap(
				identifier -> (DataResult)Optional.ofNullable((Preset)BY_IDENTIFIER.get(identifier))
