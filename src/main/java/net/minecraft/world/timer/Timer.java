...
public class Timer<T> {
	...
	private final Queue<Event<T>> events = new PriorityQueue(createEventComparator());
	...
	public void processEvents(T server, long time) {
		while (true) {
			Event<T> event = (Event<T>)this.events.peek();
	...
	public void setEvent(String name, long triggerTime, TimerCallback<T> callback) {
		if (!this.eventsByName.contains(name, triggerTime)) {
			...
			Event<T> event = new Event<>(triggerTime, this.eventCounter, name, callback);
	...
	public int remove(String name) {
		Collection<Event<T>> collection = this.eventsByName.row(name).values();
