...
public class ChunkNoiseSampler implements DensityFunction.EachApplier, DensityFunction.NoisePos {
	...
	public ChunkNoiseSampler(
		...
		this.interpolators = Lists.<DensityInterpolator>newArrayList();
		this.caches = Lists.<CellCache>newArrayList();
	 ...
	 * {@link DensityFunction.EachApplier EachApplier}.
	 * 
	...
	private DensityFunction getActualDensityFunctionImpl(DensityFunction function) {
		if (function instanceof DensityFunctionTypes.Wrapping wrapping) {
			return (DensityFunction)(switch (wrapping.type()) {
				case INTERPOLATED -> new DensityInterpolator(wrapping.wrapped());
				case FLAT_CACHE -> new FlatCache(wrapping.wrapped(), true);
				case CACHE2D -> new Cache2D(wrapping.wrapped());
				case CACHE_ONCE -> new CacheOnce(wrapping.wrapped());
				case CACHE_ALL_IN_CELL -> new CellCache(wrapping.wrapped());
			});
		} else {
			...
			if (sample) {
				for (int i = 0; i <= ChunkNoiseSampler.this.horizontalBiomeEnd; i++) {
					...
					for (int l = 0; l <= ChunkNoiseSampler.this.horizontalBiomeEnd; l++) {
						...
						int n = BiomeCoords.toBlock(m);
						this.cache[i][l] = delegate.sample(new UnblendedNoisePos(k, 0, n));
					}
				}
			}
		}
