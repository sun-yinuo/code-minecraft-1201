...
public class MineshaftStructure extends Structure {
	public static final Codec<MineshaftStructure> CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
					configCodecBuilder(instance), Type.CODEC.fieldOf("mineshaft_type").forGetter(mineshaftStructure -> mineshaftStructure.type)
				)
				.apply(instance, MineshaftStructure::new)
	);
	private final Type type;

	public MineshaftStructure(Config config, Type type) {
		super(config);
		this.type = type;
	}
	...
	public Optional<StructurePosition> getStructurePosition(Context context) {
		...
		return Optional.of(new StructurePosition(blockPos.add(0, i, 0), Either.right(structurePiecesCollector)));
	}
