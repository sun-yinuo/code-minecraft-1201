...
public class RuinedPortalStructure extends Structure {
	...
	public static final Codec<RuinedPortalStructure> CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
					configCodecBuilder(instance), Codecs.nonEmptyList(Setup.CODEC.listOf()).fieldOf("setups").forGetter(structure -> structure.setups)
				)
				.apply(instance, RuinedPortalStructure::new)
	);

	public RuinedPortalStructure(Config config, List<Setup> setups) {
		super(config);
		this.setups = setups;
	}

	public RuinedPortalStructure(Config config, Setup setup) {
	...
	public static record Setup(
		...
		public static final Codec<Setup> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
						RuinedPortalStructurePiece.VerticalPlacement.CODEC.fieldOf("placement").forGetter(Setup::placement),
						...
						Codec.BOOL.fieldOf("can_be_cold").forGetter(Setup::canBeCold),
						Codec.BOOL.fieldOf("replace_with_blackstone").forGetter(Setup::replaceWithBlackstone),
						Codecs.POSITIVE_FLOAT.fieldOf("weight").forGetter(Setup::weight)
					)
