...
public class WoodlandMansionStructure extends Structure {
	...
	public WoodlandMansionStructure(Config config) {
		super(config);
	}
	...
	public Optional<StructurePosition> getStructurePosition(Context context) {
		...
		return blockPos.getY() < 60
			? Optional.empty()
			: Optional.of(new StructurePosition(blockPos, collector -> this.addPieces(collector, context, blockPos, blockRotation)));
	}

	private void addPieces(StructurePiecesCollector collector, Context context, BlockPos pos, BlockRotation rotation) {
