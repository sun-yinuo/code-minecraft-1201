...
public class BlockLocating {
	 ...
	 */
	public static Rectangle getLargestRectangle(
		...
		for (int m = 1; m <= i; m++) {
			IntBounds intBounds = intBoundss[k - (m - 1)];
			intBoundss[k - m] = new IntBounds(
		...
		for (int q = l; q >= 0; q--) {
			for (int r = 0; r < intBoundss.length; r++) {
				IntBounds intBounds2 = intBoundss[r];
				int s = l - intBounds2.min;
				int t = l + intBounds2.max;
				is[r] = q >= s && q <= t ? t + 1 - q : 0;
			}

			Pair<IntBounds, Integer> pair = findLargestRectangle(is);
